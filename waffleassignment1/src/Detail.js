...
const Detail =  ({selectedId, setselectedId, object, changeModal, deleteModal}) => {
  //object는 선택한 메뉴만 들어 있는 array
  return(
    <div>
    {(object.length != 0) &&
    <div className = "Detail">
      <button className = "Close" onClick = {() => setselectedId(-1)}>x</button>{/*위의 x 누르면 선택 해제 */}
      ...
      <img src = {changeimg} onClick = {() => {changeModal(object[0])}}/>
      <img src = {deleteimg} onClick = {() => {deleteModal(object[0])}}/>
      {/*삭제, 수정 버튼 */}
      </div>
    </div>}
    </div>
